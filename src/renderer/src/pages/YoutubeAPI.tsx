...
const YoutubeAPI = () => {
  ...
  // ✅ Esperar a que el player esté listo antes de escuchar eventos
  useEffect(() => {
    let player: any = null
    ...
    const tryAttach = () => {
      ...
      // Plyr puede no estar listo aún, esperar el evento 'ready'
      player.on('ready', () => {
        player.on('ended', onEnded)
      })

      // Por si ya estaba listo
      if (player.ready) {
        player.on('ended', onEnded)
      }
    }

    // Pequeño delay para que plyr-react termine de inicializar
  ...
  return (
    <div className="flex p-4 rounded-2xl">
      <div className="w-[700px] h-[500px] mx-4">
        {/* ✅ Siempre montado para no perder la ref */}
